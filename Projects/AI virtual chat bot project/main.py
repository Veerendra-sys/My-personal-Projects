...
def wishMe():
    ...
    if (hour >= 0) and (hour <= 12) and ('AM' in t):
        speak(f"Good morning veerendra this is Prav, it's {day} and the time is {t}")
    elif (hour >= 12) and (hour <= 16) and ('PM' in t):
        speak(f"Good afternoon veerendra this is Prav, it's {day} and the time is {t}")
    else:
        speak(f"Good evening veerendra this is Prav, it's {day} and the time is {t}")
...
def schedule():
    day = cal_day().lower()
    speak("Veeru i am prav today's schedule is ")
    week={
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if ('facebook' in query) or ('discord' in query) or ('whatsapp' in query) or ('instagram' in query):
            ...
            closeApp(query)
        elif ("what" in query) or ("who" in query) or ("how" in query) or ("hi" in query) or ("thanks" in query) or (
                "hello" in query):
